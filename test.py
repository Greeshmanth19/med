...
API_KEY = "AIzaSyC0eirzyxMhjVTHy8YuP60K32Z9I1my8iE"
...
sf_connection = sf.connect(
    user="GREESH",
    password="Greeshmanth@775",
    account="CEJEYCW-NRB66887",  # Make sure this matches your URL
...
def verify_temp_table():
    try:
        ...
        if not table_exists:
            ...
            # Recreate the table because we have to clear the temp table for every rerun
            create_temp_table_query = """
            CREATE OR REPLACE TABLE temp (
                chunk_id INTEGER,
                chunk_text TEXT,
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                SELECT SNOWFLAKE.CORTEX.EMBED_TEXT_768('snowflake-arctic-embed-m', '{chunk_text}') as embedding
                """
...
def get_combined_context(question):
    try:
        ...
        SELECT chunk_text, VECTOR_COSINE_SIMILARITY(
            SNOWFLAKE.CORTEX.EMBED_TEXT_768('snowflake-arctic-embed-m', '{question.replace("'", "''")}'),
            text_vec
        ) AS similarity
        FROM temp
...
# Function to get current location dynamically
...
# Function to fetch hospitals based on disease name
def get_hospitals(keyword, latitude, longitude):
    gmaps = googlemaps.Client(key=API_KEY)
    results = gmaps.places(
        query=f"{keyword} hospital",
        location=(latitude, longitude),
        radius=5000  # Uses the user-selected radius
    )
...
st.sidebar.info(
    ...
    - Medical database (22,000 Rows of CSV File)
...
task = st.sidebar.radio(
    "Select Task",
    ["Ask Question", "View Data", "Upload PDF", "View Performance", "View Nearest Hospital"]
)
            ...
            with st.spinner("Generating response..."):
                try:
                    ...
                    if not response_data.empty:
                        ...
                        disease_prompt = f"""
                                Extract only the disease name from the following medical text.
                                Do not provide any explanations, context, or additional words.
                                If the text is not medical or does not mention a disease, return "None".
                        ...
                        rag_query2 = f"""
                                        SELECT snowflake.cortex.complete(
                                            'mistral-large2',
                                            '{disease_prompt.replace("'", "''")}'
                                        ) AS response
                                        """
                        response_data2 = session.sql(rag_query2).to_pandas()
                        if not response_data2.empty:
                            response2 = response_data2.iloc[0, 0] if not response_data2.empty else "None"
                            print(response2)
                            st.session_state.disease_name = response2
                            
                        end_time = datetime.now()
                        response_time = (end_time - start_time).total_seconds()
                        
                        # Get feedback scores without TruLens
                        feedback_prompt = f"""
...
elif task == "View Nearest Hospital":
    st.subheader("Find Nearest Hospitals 🏥")

    # Retrieve predicted disease from session state
    disease_name = st.session_state.get("disease_name", None)

    if disease_name and disease_name.strip().lower() != "none":
        st.success(f"Searching hospitals for: **{disease_name}**")

        latitude, longitude = get_current_location()
        if latitude and longitude:
            st.success(f"Current Location: {latitude}, {longitude}")

            # Fetch hospitals related to the detected disease within 5000 meters
            hospitals = get_hospitals(disease_name, latitude, longitude)

            if hospitals:
                # Interactive Folium Map (LARGER SIZE)
                st.subheader("Interactive Map 🌍")
                map_center = [latitude, longitude]
                m = folium.Map(location=map_center, zoom_start=13, width="100%", height="600px")  # Increased height
                ...
                folium_static(m, width=900, height=600)  # Set display size

            else:
                st.warning(f"No hospitals found for **{disease_name}** within 5000 meters.")

    else:
        st.warning("The query is not about hospitals.")